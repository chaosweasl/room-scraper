...
const KANBAN_COLUMNS = [
  ...
  { value: "applied", label: "Applied", accent: "border-green-500/40" },
  { value: "rejected", label: "Rejected", accent: "border-red-500/40" },
];
...
function ListingCard({ listing }: { listing: Listing }) {
  ...
  return (
    <Card
      className={cn(
        "relative",
        listing.priority === "high" && "border-red-500/60",
      )}
    >
      <CardHeader>
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          <span
            className={`text-xs px-2 py-0.5 rounded-full text-white ${getSourceColor(listing.source)}`}
          >
            {listing.source}
          </span>
        ...
        {listing.address &&
          ...
          listing.address.length >= 3 && (
            <p className="text-xs text-gray-400 mt-1">📍 {listing.address}</p>
          )}
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        {listing.draft_body && (
          <p className="text-xs text-gray-400 line-clamp-3 bg-gray-950/60 rounded-lg px-3 py-2 font-mono">
            {listing.draft_body}
          </p>
        )}
        ...
        <a
          ...
          className="block w-full text-center py-2 rounded-lg font-semibold text-sm bg-gray-800 hover:bg-gray-700 text-gray-300"
        >
...
export default async function Home({
  ...
  try {
    ...
    listings = result.rows.map((row) => ({
      ...
      commute_minutes: (row.commute_minutes as number) || null,
    }));
  } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-gray-100 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6 border-b border-gray-800 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-white">
                Kamer<span className="text-blue-500">Catch</span>
              </h1>
          ...
          <a
            href="/settings"
            className="text-sm bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg px-4 py-2 text-gray-300"
          >
            ⚙️ Settings
          </a>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-6">
          {[
            ...
            { label: "Rejected", value: String(countByStatus("rejected")) },
        ...
        {dbError && (
          <div className="bg-red-950/50 border border-red-900 text-red-200 p-6 rounded-xl mb-8">
          ...
          <form method="GET" className="flex flex-wrap gap-3 items-center">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-20 bg-gray-800 border border-gray-700 rounded-lg px-2 py-2 text-sm text-gray-200"
              />
            ...
            <input
              ...
              className="flex-1 min-w-[150px] bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200"
            />
            ...
            {KANBAN_COLUMNS.map((col) => {
              ...
              return (
                <div
                  key={col.value}
                  className={cn(
                    "bg-gray-900/40 border rounded-xl p-3",
                    col.accent,
                  )}
                >
                  <h2 className="text-sm font-bold text-gray-200 mb-3 uppercase tracking-wide">
                    {col.label}{" "}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {statusTab === "auto_rejected" && filtered.length > 0 && (
              <div className="col-span-full bg-amber-950/30 border border-amber-900/40 text-amber-200 p-4 rounded-xl text-sm">
                ⚠️ <strong>Shadow tab</strong> — these listings were
                auto-rejected by the triage engine (commute/rent/dealbreakers).
                Review occasionally to make sure the filter isn't too
                aggressive.
              </div>
            )}
            {filtered.map((l) => (
