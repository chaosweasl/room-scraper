...
export async function scrapeKamernet(): Promise<number> {
  console.log(
    `🔍 Starting Kamernet scraper for ${CITY} (max €${MAX_RENT}, 2026 toeslag rules)...`,
  );
  await ensureSchema();
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    console.log(
      `  Page ${page}: ${listings.length} listings (total available: ${total})`,
    );
    ...
    for (const l of listings) {
      ...
      // Build the listing URL, then derive a stable ID from it
      const url = buildListingUrl(l);
      const listingId = hashUrl(url);
      ...
      const title =
        `${typeName}${studentTag} — ${l.street}, ${l.city} — ${l.surfaceArea}m² ${furnishing}`.trim();
      ...
      const utils = l.utilitiesIncluded
        ? "Utilities included"
        : "Utilities excluded";
    ...
    if (!anyOnPage && page >= 5) {
      console.log(
        `  ⏹️ No more listings under €${MAX_RENT} — stopping at page ${page}`,
      );
      break;
    }
  ...
  console.log(
    `📊 Kamernet: ${totalSeen} listings checked, ${newCount} new found (total available: ${totalAvailable})`,
  );
  return newCount;
}
