...
function detectListingType(text: string): string {
  ...
  if (lower.includes("appartement") || lower.includes("apartment"))
    return "apartment";
  if (lower.includes("kamer") || lower.includes("room")) return "room";
  if (
    lower.includes("woning") ||
  ...
  try {
    ...
    try {
      const cookieBtn = page.locator(
        'button:has-text("Accept"), button:has-text("Accepteren"), button:has-text("Alles accepteren")',
      );
    ...
    for (const sel of selectors) {
      try {
        ...
        if (listingElements.length > 0) {
          console.log(
            `📋 Found ${listingElements.length} Pararius listings using "${sel}"`,
          );
          break;
        }
      } catch {
    ...
    if (listingElements.length === 0) {
      ...
      console.log(
        "⚠️ No standard listing selectors matched — trying page content extraction",
      );
    ...
    const listings = await page.$$eval(
      "section.listing-search-item, li.search-list__item, article.listing-search-item",
      (elements) => {
        return elements.map((el) => {
          const titleEl = el.querySelector(
            'h2, h3, .listing-search-item__title, [class*="title"]',
          );
          const addressEl = el.querySelector(
            '.listing-search-item__sub-title, [class*="address"], [class*="sub-title"]',
          );
          const priceEl = el.querySelector(
            '.listing-search-item__price, [class*="price"]',
          );
          const areaEl = el.querySelector(
            '.listing-search-item__surface-area, [class*="surface"], [class*="area"]',
          );
